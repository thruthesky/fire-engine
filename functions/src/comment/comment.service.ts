...
export class CommentService {
  /**
     ...
     * @param {string} commentId comment id to get the comment
     * @return {Promise<Comment | null>} Returns the comment data. If the comment is not found, then return null.
     *
  ...
  /**
     ...
     *
     * @param {string} postId post id of the comment
     * @param {string} commentId comment id to get the ancestor's uid. 마지막 코멘트의 id.
     * @param {string} authorUid the user uid of the comment id. 코멘트를 쓴 사람의 uid. 이 값이 지정되면 이 uid 를 제외하고 리턴한다.
     * @return {Promise<string[]>} Returns the uid of ancestors.
     *
